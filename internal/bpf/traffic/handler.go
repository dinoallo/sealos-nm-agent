...
func (h *TrafficEventHandler) handlePodEgress(ctx context.Context) error {
	...
	case record := <-h.EgressPodTrafficRecords:
		...
		if srcAddrType == modules.AddrTypePod && (dstAddrType == modules.AddrTypeWorld || dstAddrType == modules.AddrTypeHost || dstAddrType == modules.AddrTypeCiliumHost || dstAddrType == modules.AddrTypeNode) {
