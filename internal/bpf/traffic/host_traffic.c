...
struct {
  ...
  __uint(key_size, sizeof(u32));
  __uint(value_size, sizeof(u32));
} ingress_host_traffic_events SEC(".maps");
...
static __always_inline void submit_egress_traffic(struct __sk_buff *ctx) {
  ...
  marshal(&event, sk);
  bpf_perf_event_output(ctx, &egress_host_traffic_events, BPF_F_CURRENT_CPU,
...
static __always_inline void submit_ingress_traffic(struct __sk_buff *ctx) {
  ...
  sk = bpf_sk_fullsock(sk);
  marshal(&event, sk);
  bpf_perf_event_output(ctx, &ingress_host_traffic_events, BPF_F_CURRENT_CPU,
                        &event, sizeof(struct event_t));
}
...
SEC("classifier")
int ingress_host_traffic_hook(struct __sk_buff *ctx) {
